...
colnames(x) = colnames(x)[,c(grep(Args[2], colnames(x)), c(1:length(colnames(x)))[!(c(1:length(colnames(x))) %in% grep(Args[2], colnames(x)))])]
...
plot.ma.lattice(res.deseq, filename = Args[2], p = 0.05,log2fold = 0.0)
