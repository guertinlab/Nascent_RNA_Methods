...
The majority of reads should map concordantly to the genome. We expect an alignment rate above 80% for high quality libraries. As described above, we use `samtools` and `wc -l` to count concordantly aligned reads in the BAM alignment file and the pre-alignment FASTQ files and divide these values to calculate the alignment rate. We found that poor alignment rate typically arises from either poor quality sequencing or microorganism contamination of reagents/buffers. We recommend using FastQC to determine if the poor alignment is due to a problem with the FASTQ base quality scores [@andrews2017fastqc]. If the user suspects that contamination of another species' DNA is causing poor alignment, we recommend using `BLAST` to query unaligned sequences to genome databases [@altschul1997gapped]. The user can leverage the sequences to design PCR primers for the contaminating species and test reagents to identify the culprit.
