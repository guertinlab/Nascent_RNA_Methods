...
The proportion of reads that are adapter/adapter ligation products varies widely depending upon whether a size selection was performed in the library preparation. We recently excluded the size selection step from the PRO-seq protocol to reduce bias against small RNA inserts [@sathyan2019improved; @smith2021peppro]. If no size selection is performed, the adapter/adapter ligation fraction can be quite high; however, a high value does not necessarily indicate poor library quality. If all other QC metrics indicate that the data is high quality, the user should balance to cost of performing another experiment with sequencing uninformative adapter sequences. In a later section we provide a formula for determining the required sequencing depth to result in a desired number of concordant aligned reads. If there are too few concordantly aligned reads, we recommend further sequencing, unless more than 80% of the reads are adapter/adapter ligation products. 
