...
Later on we provide a formula for determining the required sequncing depth to result in a desired number of concordant aligned reads. So while the percentage of uninformative adapter reads is a useful number, if all other QC metrics indicate that the data is high quality, then we recommend further sequencing depth if less than 80% of the reads are adapter/adapter ligation products. 
